...
503 Service Unavailable 表示服务端进程正在启动但尚未就绪，或启动过程中崩溃。
...
# 手动测健康检查
docker exec sena-repo python3 -c "import urllib.request; print(urllib.request.urlopen('http://localhost:11451/api/health').read())"
```

常见原因：

1. 数据库文件损坏，启动时初始化失败
2. 容器内存不足（`docker stats sena-repo` 查看）
3. 首次启动正在执行全量扫描，CPU/IO 占用高导致健康检查超时
4. `config.yaml` 中的 `games_path` 指向不存在的路径
