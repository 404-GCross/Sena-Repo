...
- 🖼️ **私有视觉小说资源库** — 扫描本地目录或 OpenList 文件源，按会社、游戏、版本整理资源，并在客户端以网格/列表展示
- 🌐 **OpenList 直链下载** — OpenList 文件源支持 `/d/...` 302 跳转到网盘直链，客户端下载时不走 Sena 服务端带宽
- 📦 **下载与解压** — 客户端支持断点续传、并发数设置、下载目录设置、总限速、解压密码预填入、打开目标文件夹
- 🎮 **Steam 补丁注入** — 扫描匹配补丁文件，为 Steam 已购视觉小说注入汉化、音声、剧情或额外内容补丁
- 🔎 **多源元数据刮削** — 支持 VNDB、Bangumi、Steam、月幕 GalGame，并可通过 VNDB / Bangumi ID 精确刮削
- 👥 **多用户与审批** — 首位用户为管理员，后续注册需管理员审批，可管理头像、权限和通知
