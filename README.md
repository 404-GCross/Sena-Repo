...
- 📂 **目录扫描** — 扫码会社/游戏/压缩包三级目录，自动识别平台与版本
- 🏷️ **文件名清洗** — 从压缩包文件名提取游戏名、平台标识，过滤非游戏文件
- 🎮 **Steam 补丁注入** — 服务端管理补丁压缩包，关键词快捷匹配类型，可批量注入
- 🐳 **Docker 一键部署** — GHCR 镜像拉取即用，挂载目录持久化数据

**客户端**

- 🖼️ **游戏库浏览** — 网格/列表双视图，按会社/标签/平台筛选，支持搜索与排序
- ✏️ **元数据编辑** — 多源元数据逐字段对比勾选下载，封面上传即时生效
- 📥 **游戏下载** — 暂停/取消/断点续传，下载完自动解压，Android 适配外部存储
- 👥 **用户系统** — 注册需管理员审批，支持头像上传，多用户独立管理
- 🧭 **初始化向导** — 首次连接自动创建管理员账号并配置游戏目录
- 🎨 **个性化** — 主题色切换、最小化系统托盘、双击图标拉起已运行实例
- 💻 **Windows 安装包** — 安装版 exe（开始菜单+卸载入口）与便携 zip 双版本
