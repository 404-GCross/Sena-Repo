...
- 📂 **目录扫描** — 根据你选择的目录模式，自动清洗文件名并归类平台与版本
...
Linux AppImage 已针对 Wayland / Gamescope / Bazzite 等掌机环境做触控兼容处理。

- 启动时会优先使用 `GDK_BACKEND=wayland,x11`
- Linux runner 会输出输入诊断日志，例如 `Sena Linux input backend preference`、`Sena Linux GTK display`
- 如果终端中能看到 `Sena Linux touch event`，说明系统触摸事件已经进入 GTK
- 单指触摸会桥接为鼠标左键点击/拖动，以兼容 Flutter Linux 桌面端触控行为
