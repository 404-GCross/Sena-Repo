...
class _ConnectScreenState extends State<ConnectScreen> {
  ...
  Future<void> _connectToProfile(UserProfile profile, int index) async {
    ...
    try {
      final settings = context.read<SettingsProvider>();
      final games = context.read<GameProvider>();
      final success = await settings.connect(
        profile.host,
        profile.port,
        useHttps: profile.useHttps,
      );
      ...
      try {
        ...
        if (mounted) {
          setState(() {
            ...
            _error = "连接失败: ${msg.length > 80 ? msg.substring(0, 80) : msg}";
          });
        }
      }
    } catch (e) {
      ...
      if (mounted)
        _showToast(
          "连接失败: ${e.toString().length > 100 ? e.toString().substring(0, 100) : e}",
        );
    }
  }
  ...
  Future<void> _handleResetServerSetup(
    ...
    try {
      ...
      try {
        ...
        final msg =
            "初始化成功，但加载游戏库失败: ${text.length > 80 ? text.substring(0, 80) : text}";
        _showToast(msg);
        setState(() => _error = msg);
      }
    } catch (e) {
      ...
      final text = e.toString();
      final msg =
          "初始化后登录失败: ${text.length > 80 ? text.substring(0, 80) : text}";
      _showToast(msg);
      setState(() => _error = msg);
    }
  }
  ...
  void _showConnectingDialog(UserProfile profile, VoidCallback onCancel) {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [TextButton(onPressed: onCancel, child: const Text("取消"))],
      ),
    );
  }

  Future<void> _syncProfileFromMeResponse(
    UserProfile profile,
    int index,
    String body,
  ) async {
          ...
          content: SizedBox(
            ...
            child: Column(
              ...
              children: [
                ...
                TextField(
                  controller: hostCtrl,
                  decoration: const InputDecoration(labelText: "服务器IP"),
                ),
                const SizedBox(height: 8),
                TextField(
                  controller: portCtrl,
                ...
                SwitchListTile(
                  ...
                  value: useHttps,
                  onChanged: (v) => setD(() => useHttps = v),
                  dense: true,
                  contentPadding: EdgeInsets.zero,
                ),
                TextField(
                  controller: userCtrl,
                  decoration: const InputDecoration(labelText: "用户名（留空不修改）"),
                ),
                const SizedBox(height: 8),
                TextField(
                  controller: passCtrl,
            ...
            FilledButton(
              onPressed: () async {
                if (nameCtrl.text.trim().isEmpty ||
                    hostCtrl.text.trim().isEmpty) return;
                ...
                if (passCtrl.text.isNotEmpty) {
                  ...
                  try {
                    final resp = await http.post(
                      Uri.parse(
                        "$scheme://${hostCtrl.text.trim()}:$port/api/auth/login",
                      ),
                      headers: {"Content-Type": "application/json"},
                      body: jsonEncode({
              ...
              onPressed: loading
                  ...
                  : () async {
                      if (step == 0) {
                        ...
                        final port =
                            int.tryParse(portCtrl.text.trim()) ?? 11451;
                        if (host.isEmpty) {
                          setD(() => error = "请输入服务器地址");
                          return;
                        }
                        ...
                        final settings = context.read<SettingsProvider>();
                        final ok = await settings.connect(
                          host,
                          port,
                          useHttps: useHttps,
                        );
                        if (!ok) {
                          setD(() {
                            loading = false;
                            error = settings.errorMessage ?? "连接失败";
                          });
                          return;
                        }
                        ...
                        if (needsSetup) {
                          ...
                          final setupResult = await Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (_) => SetupWizardScreen(api: api!),
                            ),
                          );
                          if (setupResult != null && mounted) {
                            final creds = setupResult as Map;
                            ...
                            if (loginResult != null) {
                              ...
                              await _goHome(
                                games: context.read<GameProvider>(),
                              );
                            }
                          }
                          return;
                        ...
                        if (userCtrl.text.trim().isEmpty) {
                          setD(() => loginError = "请输入用户名");
                          return;
                        }
                        if (passCtrl.text.length < 4) {
                          setD(() => loginError = "密码至少4位");
                          return;
                        }
                        if (passCtrl.text != passConfirmCtrl.text) {
                          setD(() => loginError = "两次密码不一致");
                          return;
                        }
                        setD(() {
                          loading = true;
              ...
              children: [
                ...
                Text(
                  ...
                  style: AppText.label.copyWith(
                    color: path.isEmpty ? Colors.red[300] : Colors.grey[600],
                  ),
                ),
              ],
            ),
