...
class _ProfileSwitchScreenState extends State<ProfileSwitchScreen> {
  ...
  Future<void> _showProfileEditor(UserProfile? existing) async {
    ...
    final hostCtrl =
        TextEditingController(text: existing?.host ?? "192.168.1.100");
    final portCtrl =
        TextEditingController(text: (existing?.port ?? 11451).toString());
                        ...
                        children: [
                          TextField(
                              ...
                              decoration: const InputDecoration(
                                  labelText: "配置名称", hintText: "如: 家里NAS")),
                          const SizedBox(height: 8),
                          TextField(
                              controller: hostCtrl,
                              decoration:
                                  const InputDecoration(labelText: "服务器 IP")),
                          const SizedBox(height: 8),
                          TextField(
                              controller: portCtrl,
                              decoration:
                          ...
                          TextField(
                              ...
                              decoration: InputDecoration(
                                  labelText: isEdit ? "用户名（留空不修改）" : "用户名")),
                          const SizedBox(height: 8),
                          TextField(
                              controller: passCtrl,
                              decoration: InputDecoration(
                      ...
                      onPressed: () async {
                        ...
                        try {
                          ...
                          if (resp.statusCode != 200) {
                            ...
                            final detail =
                                err == null ? null : err["detail"]?.toString();
  ...
  Future<void> saveEditedProfile(
      UserProfile newProfile, UserProfile oldProfile) async {
    final ps = ProfileService();
    final profiles = await ps.loadProfiles();
  ...
  Widget build(BuildContext context) {
    return AppScaffold(
      title: "切换用户",
      subtitle: "选择已保存的服务器配置和账号会话",
      leading: const Icon(Icons.people_alt_outlined, size: 24),
      scrollable: false,
      ...
      child: _loading
          ? const AppStateView.loading(title: "正在读取用户配置")
          : _profiles.isEmpty
              ? const AppStateView(
                  icon: Icons.people_outline,
                  title: "暂无保存的配置",
                  message: "点击右下角按钮新增服务器和账号配置",
                )
              : ListView.builder(
